...
namespace TerraUnity.UI
{
    public class TerraWorld : EditorWindow
    {
...
#else
        [MenuItem("Tools/TerraUnity/TerraWorld Lite", false, 1)]
#endif
        static void Init()
        {
        ...
        //private int dayNightCycleIndex = 0;
        ...
        private void InitSceneVisuals()
        {
#if TERRAWORLD_PRO
            if (TTerraWorldManager.SceneSettingsGO1 == null) return;
            //FXNode fxNode = TTerraWorld.WorldGraph.FXGraph.GetEntryNode();
            //InitFXSettings(SceneSettingsManager.FXParameters);
            //SceneSettingsManager.UpdateWaterColors(SceneSettingsManager.FXParameters);
            //SceneSettingsManager.UpdateWaterRendering(SceneSettingsManager.FXParameters);
#endif
        }
        ...
        protected virtual void OnDisable()
        {
            ...
            ClearHelperObjects();
            if (!TTerraWorldManager.isQuitting) TTerraWorld.SaveWorldGraph();
        ...
        public void OnGUI()
        {
            ...
            if (TTerraWorldManager.MainTerraworldGameObject == null)
            {
                ShowNotification(new GUIContent("No TerraWorld Initialized", ""));
                return;
            }
        ...
        private void GenerateWorldGUI()
        {
            ...
            if (TAreaPreview.Visible)
                ...
                GUI.color = new UnityEngine.Color(0.5f, 0.5f, 0.5f, 0.5f);
        ...
        private void WorldAreaSettings(ref WorldArea module)
        {
            ...
            if (Event.current.shift && Event.current.control)
            {
                ...
                if (GUILayout.Button(new GUIContent("Save"), GUILayout.Width(55), GUILayout.Height(55)))
                {
                    ...
                    {
                        TTerraWorld.WorldGraph.SaveGraphAsTemplate(graphPath);
            ...
            else
            {
                ...
                if (GUILayout.Button(new GUIContent(TResourcesManager.saveIcon, "Save current graph into preset file"), GUILayout.Width(55), GUILayout.Height(55)))
                {
                    ...
                    if (!string.IsNullOrEmpty(graphPath) && Directory.Exists(Path.GetDirectoryName(graphPath)) && graphPath.EndsWith(".xml"))
                    {
                        ...
                        TTerraWorld.SaveWorldGraph (graphPath);
                        AssetDatabase.Refresh();
                    }
                    else
                    {
...
#if TERRAWORLD_XPRO
                TTerraworldGenerator.ResetXGraph();
#endif
...
//     //           TTerraWorld.LoadWorldGraph(DemoPath, false, out exception, out bool reGenerate);
       ...
       //     if (exception != null) throw exception;
        ...
        private void TerrainRenderingSettings(ref RenderingNode module)
        {
            ...
            {
                TerrainRenderingModeUI();
#if TERRAWORLD_PRO
                if (TerrainRenderingManager.isModernRendering)
                    TerrainRenderingUI(ref module);
#endif
            }

            SplatmapsUI(ref module);
            MainTerrainUI(ref module);
            BackgroundTerrainUI(ref module);
        ...
        private void TerrainRenderingModeUI()
        {
            GUIStyle style = new GUIStyle(EditorStyles.toolbarButton);
            ...
            terrainRenderingIndex = Convert.ToInt32(!TerrainRenderingManager.isModernRendering);
            EditorGUI.BeginChangeCheck();
            terrainRenderingIndex = THelpersUI.GUI_SelectionGrid(terrainRenderingIndex, renderingMode, style, -10);

            if (EditorGUI.EndChangeCheck())
            {
                TerrainRenderingManager.isModernRendering = !Convert.ToBoolean(terrainRenderingIndex);
        ...
        private void TerrainRenderingUI(ref RenderingNode module)
        {
            int previewSize = 128;
            int padLeft = 150;
            int extraSpace = -10;
            ...
            if (THelpersUI.SectionSettingsWithTitle(ref module.sectionToggles.Settings1, "RENDERING"))
            {
                ...
                TerrainRenderingManager.LightingColor = THelpersUI.GUI_ColorField(new GUIContent("TERRAINS COLOR TINT", "Color tint on main terrain surface"), TerrainRenderingManager.LightingColor, 20);
                TerrainRenderingManager.LightingColorBG = THelpersUI.GUI_ColorField(new GUIContent("BACKGROUND TERRAINS COLOR TINT", "Color tint on background terrain surface"), TerrainRenderingManager.LightingColorBG, 20);
               ...
               // }
                ...
                terrainTessellationIndex = Convert.ToInt32(!TerrainRenderingManager.isTessellated);
                EditorGUI.BeginChangeCheck();
                terrainTessellationIndex = THelpersUI.GUI_SelectionGrid(terrainTessellationIndex, onOffSelection, style, -10);
                if (EditorGUI.EndChangeCheck())
                {
                    TerrainRenderingManager.isTessellated = !Convert.ToBoolean(terrainTessellationIndex);

                    if (TerrainRenderingManager.isTessellated)
                ...
                if (TerrainRenderingManager.isTessellated)
                {
                    ...
                    if (THelpersUI.SectionSettings(ref module.uIToggles.Settings1))
                    {
                      ...
                      //  TerrainRenderingManager.TerrainMaterial.SetFloat("_EdgeLength", _FloatValue);
                        ...
                        TerrainRenderingManager.TessellationQuality = THelpersUI.GUI_Slider(new GUIContent("QUALITY", "Tessellation quality on surface"), TerrainRenderingManager.TessellationQuality, 2f, 64f);
                      ...
                      //  TerrainRenderingManager.TerrainMaterial.SetFloat("_Phong", _FloatValue);
                        ...
                        TerrainRenderingManager.EdgeSmoothness = THelpersUI.GUI_Slider(new GUIContent("EDGE SMOOTHNESS", "Phong smoothness on surface edges"), TerrainRenderingManager.EdgeSmoothness, 0f, 10f, extraSpace);
                    }
                }
                ...
                terrainHeightBlendingIndex = Convert.ToInt32(!TerrainRenderingManager.isHeightmapBlending);
                EditorGUI.BeginChangeCheck();
                terrainHeightBlendingIndex = THelpersUI.GUI_SelectionGrid(terrainHeightBlendingIndex, onOffSelection, style, -10);
                if (EditorGUI.EndChangeCheck())
                {
                    TerrainRenderingManager.isHeightmapBlending = !Convert.ToBoolean(terrainHeightBlendingIndex);

                    if (TerrainRenderingManager.isHeightmapBlending)
                ...
                EditorGUI.BeginDisabledGroup(!TerrainRenderingManager.isHeightmapBlending);
                if (THelpersUI.SectionSettings(ref module.uIToggles.Settings2))
                {
                    EditorGUI.BeginChangeCheck();
                    _FloatValue = TerrainRenderingManager.HeightmapBlending;
                    ...
                    if (EditorGUI.EndChangeCheck()) TerrainRenderingManager.HeightmapBlending = _FloatValue;
                }
                ...
                terrainColormapBlendingIndex = Convert.ToInt32(!TerrainRenderingManager.isColormapBlending);
                EditorGUI.BeginChangeCheck();
                terrainColormapBlendingIndex = THelpersUI.GUI_SelectionGrid(terrainColormapBlendingIndex, onOffSelection, style, -10);
                if (EditorGUI.EndChangeCheck())
                {
                    TerrainRenderingManager.isColormapBlending = !Convert.ToBoolean(terrainColormapBlendingIndex);
                ...
                EditorGUI.BeginDisabledGroup(!TerrainRenderingManager.isColormapBlending);
                if (THelpersUI.SectionSettings(ref module.uIToggles.Settings3))
                {
                    ...
                    if (TerrainRenderingManager.ColormapTexture != null)
                    {
                        ...
                        EditorGUI.BeginChangeCheck();
                        _FloatValue = TerrainRenderingManager.BlendingDistance;
                        ...
                        if (EditorGUI.EndChangeCheck()) TerrainRenderingManager.BlendingDistance = _FloatValue;
                    }
                    else
                ...
                terrainSnowIndex = Convert.ToInt32(!TerrainRenderingManager.isProceduralSnow);
                EditorGUI.BeginChangeCheck();
                terrainSnowIndex = THelpersUI.GUI_SelectionGrid(terrainSnowIndex, onOffSelection, style, -10);
                if (EditorGUI.EndChangeCheck())
                {
                    TerrainRenderingManager.isProceduralSnow = !Convert.ToBoolean(terrainSnowIndex);

                    if (TerrainRenderingManager.isProceduralSnow)
                ...
                EditorGUI.BeginDisabledGroup(!TerrainRenderingManager.isProceduralSnow);
                if (THelpersUI.SectionSettings(ref module.uIToggles.Settings4))
                {
                    ...
                    // Snow color
                    EditorGUI.BeginChangeCheck(); 
                    _Color = TerrainRenderingManager.SnowColor;
                    _Color = THelpersUI.GUI_ColorField(new GUIContent("SNOW COLOR", "Snow color on terrain surface"), _Color, 20, padLeft);
                    if (EditorGUI.EndChangeCheck()) TerrainRenderingManager.SnowColor = _Color;
                    ...
                    EditorGUI.BeginChangeCheck();
                    _FloatValue = TerrainRenderingManager.SnowTile;
                    ...
                    if (EditorGUI.EndChangeCheck()) TerrainRenderingManager.SnowTile = _FloatValue;

                    EditorGUI.BeginChangeCheck();
                    _FloatValue = TerrainRenderingManager.SnowAmount;
                    ...
                    if (EditorGUI.EndChangeCheck()) TerrainRenderingManager.SnowAmount = _FloatValue;

                    EditorGUI.BeginChangeCheck();
                    _FloatValue = TerrainRenderingManager.SnowAngle;
                    ...
                    if (EditorGUI.EndChangeCheck()) TerrainRenderingManager.SnowAngle = _FloatValue;

                    EditorGUI.BeginChangeCheck();
                    _FloatValue = TerrainRenderingManager.SnowNormalInfluence;
                    ...
                    if (EditorGUI.EndChangeCheck()) TerrainRenderingManager.SnowNormalInfluence = _FloatValue;

                    EditorGUI.BeginChangeCheck();
                    _FloatValue = TerrainRenderingManager.SnowPower;
                    ...
                    if (EditorGUI.EndChangeCheck()) TerrainRenderingManager.SnowPower = _FloatValue;

                    EditorGUI.BeginChangeCheck();
                    _FloatValue = TerrainRenderingManager.SnowSmoothness;
                    ...
                    if (EditorGUI.EndChangeCheck()) TerrainRenderingManager.SnowSmoothness = _FloatValue;
                ...
                terrainPuddlesIndex = Convert.ToInt32(!TerrainRenderingManager.isProceduralPuddles);
                EditorGUI.BeginChangeCheck();
                terrainPuddlesIndex = THelpersUI.GUI_SelectionGrid(terrainPuddlesIndex, onOffSelection, style, -10);
                if (EditorGUI.EndChangeCheck())
                {
                    TerrainRenderingManager.isProceduralPuddles = !Convert.ToBoolean(terrainPuddlesIndex);
                ...
                EditorGUI.BeginDisabledGroup(!TerrainRenderingManager.isProceduralPuddles);
                if (THelpersUI.SectionSettings(ref module.uIToggles.Settings13))
                {
                    ...
                    EditorGUI.BeginChangeCheck(); 
                    _Color = TerrainRenderingManager.PuddleColor;
                    _Color = THelpersUI.GUI_ColorField(new GUIContent("PUDDLE COLOR", "Puddle color on terrain surface"), _Color, 0, padLeft);
                    if (EditorGUI.EndChangeCheck()) TerrainRenderingManager.PuddleColor = _Color;

                    EditorGUI.BeginChangeCheck();
                    _FloatValue = TerrainRenderingManager.PuddleRefraction;
                    _FloatValue = THelpersUI.GUI_Slider(new GUIContent("REFRACTION", "Refraction value for puddle rendering"), _FloatValue, 0f, 1f, extraSpace, padLeft);
                    if (EditorGUI.EndChangeCheck()) TerrainRenderingManager.PuddleRefraction= _FloatValue;

                    EditorGUI.BeginChangeCheck();
                    _FloatValue = TerrainRenderingManager.PuddleMetallic;
                    _FloatValue = THelpersUI.GUI_Slider(new GUIContent("METALLIC", "Metallic value for puddle rendering"), _FloatValue, 0f, 1f, extraSpace, padLeft);
                    if (EditorGUI.EndChangeCheck()) TerrainRenderingManager.PuddleMetallic = _FloatValue;

                    EditorGUI.BeginChangeCheck();
                    _FloatValue = TerrainRenderingManager.PuddleSmoothness;
                    _FloatValue = THelpersUI.GUI_Slider(new GUIContent("SMOOTHNESS", "Smoothness value for puddle rendering"), _FloatValue, 0f, 1f, extraSpace, padLeft);
                    if (EditorGUI.EndChangeCheck())
                        TerrainRenderingManager.PuddleSmoothness = _FloatValue;

                    EditorGUI.BeginChangeCheck();
                    _FloatValue = TerrainRenderingManager.PuddleSlope;
                    _FloatValue = THelpersUI.GUI_Slider(new GUIContent("MAX. SLOPE", "Maximum slope value for puddle rendering"), _FloatValue, 0.0000001f, 0.01f, extraSpace, padLeft);
                    if (EditorGUI.EndChangeCheck()) TerrainRenderingManager.PuddleSlope = _FloatValue;

                    EditorGUI.BeginChangeCheck();
                    _FloatValue = TerrainRenderingManager.PuddleSlopeMin;
                    _FloatValue = THelpersUI.GUI_Slider(new GUIContent("MIN. SLOPE", "Minimum slope value for puddle rendering"), _FloatValue, 0f, 0.01f, extraSpace, padLeft);
                    if (EditorGUI.EndChangeCheck()) TerrainRenderingManager.PuddleSlopeMin = _FloatValue;

                    EditorGUI.BeginChangeCheck();
                    _FloatValue = TerrainRenderingManager.PuddleNoiseTiling;
                    _FloatValue = THelpersUI.GUI_Slider(new GUIContent("NOISE TILING", "Noise tiling value for puddle rendering"), _FloatValue, 0f, 1000f, extraSpace, padLeft);
                    if (EditorGUI.EndChangeCheck()) TerrainRenderingManager.PuddleNoiseTiling = _FloatValue;
                ...
                if (TerrainRenderingManager.TerrainLayersCount == 0)
                    GUI.color = enabledColor;
                else if (TerrainRenderingManager.isModernRendering)
                ...
                if (TerrainRenderingManager.WorldTerrain != null && TerrainRenderingManager.isModernRendering && TerrainRenderingManager.WorldTerrain.terrainData != null)
                {
                    int sectionSpace = 50;
                    style = new GUIStyle(); style.fixedWidth = previewSize; style.fixedHeight = previewSize;
                    ...
                    for (int i = 0; i < TerrainRenderingManager.TerrainLayersCount; i++)
                    {
                        ...
                        if (layer != null)
                        {
                            ...
                            if (THelpersUI.SectionSettings(ref toggles[i], padLeft))
                            {
                                ...
                                {
                                    EditorGUI.BeginChangeCheck();
                                    _FloatValue = TerrainRenderingManager.GetDisplacement(i);
                                    _FloatValue = THelpersUI.GUI_Slider(new GUIContent("DISPLACEMENT", "Tessellation displacement taken from layer heightmap on surface"), _FloatValue, 0.1f, 32f, extraSpace, padLeft);
                                    if (EditorGUI.EndChangeCheck()) TerrainRenderingManager.SetDisplacement(i , _FloatValue);
                                    ...
                                    EditorGUI.BeginChangeCheck();
                                    _FloatValue = TerrainRenderingManager.GetHeightOffset(i);
                                    _FloatValue = THelpersUI.GUI_Slider(new GUIContent("HEIGHT OFFSET" + (i + 1), "Layer height offset on surface"), _FloatValue, 0.1f, 8f, extraSpace, padLeft);
                                    if (EditorGUI.EndChangeCheck()) TerrainRenderingManager.SetHeightOffset(i, _FloatValue);
                                }

                                EditorGUI.BeginChangeCheck();
                                _FloatValue = TerrainRenderingManager.GetTileRemover(i);
                                _FloatValue = THelpersUI.GUI_Slider(new GUIContent("TILING REMOVER" + (i + 1), "Tiling Remover intensity"), _FloatValue, 0f, 64f, extraSpace, padLeft);
                                if (EditorGUI.EndChangeCheck()) TerrainRenderingManager.SetTileRemover(i, _FloatValue);

                                EditorGUI.BeginChangeCheck();
                                _FloatValue = TerrainRenderingManager.GetNoiseTiling(i);
                                _FloatValue = THelpersUI.GUI_Slider(new GUIContent("TILING NOISE" + (i + 1), "Tiling Remover noise"), _FloatValue, 0f, 200f, extraSpace, padLeft);
                                if (EditorGUI.EndChangeCheck()) TerrainRenderingManager.SetNoseTiling(i, _FloatValue);

                                EditorGUI.BeginChangeCheck();
                                _Color = TerrainRenderingManager.GetLayerColor(i);
                                _Color = THelpersUI.GUI_ColorField(new GUIContent("COLOR" + (i + 1), "Layer color on terrain surface"), _Color, extraSpace, padLeft);
                                if (EditorGUI.EndChangeCheck()) TerrainRenderingManager.SetLayerColor(i, _Color);

                                EditorGUI.BeginChangeCheck();
                                _FloatValue = TerrainRenderingManager.GetLayerAO(i);
                                _FloatValue = THelpersUI.GUI_Slider(new GUIContent("AO" + (i + 1), "Ambient Occlusion value for texture layer rendering"), _FloatValue, 0f, 1f, extraSpace, padLeft);
                                if (EditorGUI.EndChangeCheck()) TerrainRenderingManager.SetLayerAO(i, _FloatValue);
                            }
                        }
        ...
        private void SplatmapsUI(ref RenderingNode module)
        {
            //GUIStyle style = new GUIStyle(EditorStyles.toolbarButton);
            ...
            //if (THelpersUI.SectionSettingsWithTitle(ref module.sectionToggles.Settings2, "SPLATMAPS"))
            {
                ...
                TerrainRenderingManager.SplatmapResolutionBestFit = THelpersUI.GUI_Toggle(new GUIContent("BEST FIT", "Automatically calculate splatmap resolution"), TerrainRenderingManager.SplatmapResolutionBestFit);
                if (!TerrainRenderingManager.SplatmapResolutionBestFit) TerrainRenderingManager.SplatmapResolution = THelpersUI.GUI_IntSlider(new GUIContent("RESOLUTION", "Splatmap Resolution for texture painting on terrain"), Mathf.ClosestPowerOfTwo(TerrainRenderingManager.SplatmapResolution), 64, 2048, -10);
                TerrainRenderingManager.SplatmapSmoothness = THelpersUI.GUI_IntSlider(new GUIContent("SMOOTHNESS", "Smoothness iterations for the splatmap channels blending"), TerrainRenderingManager.SplatmapSmoothness, 0, 4, -10);
            }
        }
        ...
        private void BackgroundTerrainUI(ref RenderingNode module)
        {
            ...
            //if (THelpersUI.SectionSettingsWithTitle(ref module.sectionToggles.Settings4, "BACKGROUND TERRAIN"))
            {
                ...
                selectionIndexBGMountains = TerrainRenderingManager.BGMountains ? 0 : 1;
                selectionIndexBGMountains = THelpersUI.GUI_SelectionGrid(selectionIndexBGMountains, onOffSelection, style);
                TerrainRenderingManager.BGMountains = selectionIndexBGMountains == 0 ? true : false;

                if (TerrainRenderingManager.BGMountains)
                {
                    if (THelpersUI.SectionSettings(ref module.uIToggles.Settings1))
                    {
                        TerrainRenderingManager.BGTerrainScaleMultiplier = THelpersUI.GUI_IntSlider(new GUIContent("AREA MULTIPLIER", "Background Terrain's area size is value multiplied with the original world size"), TerrainRenderingManager.BGTerrainScaleMultiplier, 2, 10, -10);
                        TerrainRenderingManager.BGTerrainHeightmapResolution = THelpersUI.GUI_IntSlider(new GUIContent("HEIGHTMAP RESOLUTION", "Select heightmap resolution for the background world"), Mathf.ClosestPowerOfTwo(TerrainRenderingManager.BGTerrainHeightmapResolution), 128, 8192, -10);
                        TerrainRenderingManager.BGTerrainSatelliteImageResolution = THelpersUI.GUI_IntSlider(new GUIContent("IMAGE RESOLUTION", "Select satellite image resolution for the background world"), Mathf.ClosestPowerOfTwo(TerrainRenderingManager.BGTerrainSatelliteImageResolution), 32, 4096, -10);
                        TerrainRenderingManager.BGTerrainPixelError = THelpersUI.GUI_IntSlider(new GUIContent("PIXEL ERROR", "Pixel Error factor for the background terrain surface quality"), TerrainRenderingManager.BGTerrainPixelError, 1, 200, -10);
                        TerrainRenderingManager.BGTerrainOffset = THelpersUI.GUI_FloatField(new GUIContent("Y OFFSET", "Offset in meters for the background terrain's Y position"), TerrainRenderingManager.BGTerrainOffset, -2000, 2000, -10);
                    }
                }
            }
        }
        ...
        private void SwitchVFX(FXParams fxParams)
        {
            if (TTerraWorldManager.SceneSettingsGO1 == null) return;
            SceneSettingsManager.SwitchFX(fxParams);
        ...
        private void VFXSettings(ref FXNode module)
        {
          ...
          //      fXParamsUI.selectionIndexVFX = 0;
            ...
            //dayNightCycleIndex = fXParamsUI.dayNightControl;
            ...
            //    SceneSettingsManager.SwitchDayNightControl(fXParamsUI);
            //}
            ...
            if (fXParamsUI.dayNightControl != 0)
            {
                if (THelpersUI.SectionSettings(ref module.uIToggles.Settings1))
                {
                    ...
                    //        TTerraWorldManager.TimeOfDayManagerScript.OnValidate();
            ...
            if (THelpersUI.SectionSettings(ref module.uIToggles.Settings2))
            {
                ...
                //if (TTerraWorldManager.CloudsManagerScript != null) TTerraWorldManager.CloudsManagerScript.altitude = fXParamsUI.cloudsAltitude;
                ...
                //if (EditorGUI.EndChangeCheck()) SceneSettingsManager.UpdateClouds(fXParamsUI);
                ...
                if (EditorGUI.EndChangeCheck())
                {
                    fXParamsUI.cloudShadows = !Convert.ToBoolean(cloudShadowsIndex);
                    //SceneSettingsManager.UpdateClouds(fXParamsUI);
                }
                ...
                if (EditorGUI.EndChangeCheck())
                {
                    fXParamsUI.cloudColor = new System.Numerics.Vector4(tempColor.r, tempColor.g, tempColor.b, tempColor.a);
                    //SceneSettingsManager.UpdateClouds(fXParamsUI);
                }
                ...
                //if (EditorGUI.EndChangeCheck()) SceneSettingsManager.SwitchClouds(fXParamsUI);
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ...
                TerrainRenderingManager.isProceduralSnow = fXParamsUI.hasSnow;
            ...
            if (THelpersUI.SectionSettings(ref module.uIToggles.Settings7))
            {
                ...
                {
                    _BoolValue = TerrainRenderingManager.isProceduralSnowBG;
                    EditorGUI.BeginChangeCheck();
            ...
            {
                EditorGUI.BeginDisabledGroup(!TerrainRenderingManager.isModernRendering);
                {
                    style = new GUIStyle(EditorStyles.toolbarButton);
                    polyStyleTerrainIndex = Convert.ToInt32(!TerrainRenderingManager.isFlatShading);
                    ...
                    if (EditorGUI.EndChangeCheck()) 
                        TerrainRenderingManager.isFlatShading = !Convert.ToBoolean(polyStyleTerrainIndex);
                }
                EditorGUI.EndDisabledGroup();
            ...
            {
                SceneSettingsManager.FXParameters = fXParamsUI;
                SceneSettingsManager.UpdateParams(fXParamsUI, updateClouds);
                TTerraWorld.SaveWorldGraph();
            }

            //if (SceneSettingsManager.globalTimeManager != null)
            //{
            //    //SceneSettingsManager.globalTimeManager.fxParams = fXParamsUI;
            //    //SceneSettingsManager.globalTimeManager.UpdateParams();
            //    SceneSettingsManager.UpdateParams(fXParamsUI);
            //}
        ...
        private void GlobalSettings()
        {
            ...
            THelpersUI.GUI_HelpBoxTitleWide(new GUIContent("DEBUG LOG", "Console logger"), 10, enabledColor, THelpersUI.UIColor);
            ...
            TProjectSettings.ErrorLog = THelpersUI.GUI_Toggle(new GUIContent("Use Error Log Window", "Show Graph Errors In Error Log Window"), TProjectSettings.ErrorLog, 10);
        ...
        private void DisplayPlayerTargets()
        {
            ...
            for (int i = 0; i < InteractiveTargets.playerTargets.Count; i++)
            {
                ...
                if (playerInteractions.GPULayers != null && playerInteractions.GPULayers.Count > 0)
                {
                    ...
                    {
                        playerInteractions.maximumObjects = THelpersUI.GUI_IntSlider(new GUIContent("MAXIMUM INTERACTIVE OBJECTS", "Maximum count for interactive objects around target for each layer which will be updated dynamically in runtime"), playerInteractions.maximumObjects, 1, 128, 20);
                    ...
                    for (int j = 0; j < playerInteractions.GPULayers.Count; j++)
                    {
                        ...
                        EditorGUI.BeginChangeCheck();
                        state = THelpersUI.GUI_SelectionGrid(state, convertSelection, new GUIStyle(EditorStyles.toolbarButton), -10);
        ...
        private void TerraceSettings(ref TerraceProcess module)
        {
            ...
            if (THelpersUI.Foldout("SETTINGS", ref module.uIToggles.Settings1))
            {
                ...
                GUI.backgroundColor = new Color(0, 0.9f, 0, 0.1f);
                ...
                {
                    module.controlPoints = THeightmapProcessors.AutoSetTerraces(module._terraceCount, module._terraceVariation, 0, 1);
                }
                GUILayout.FlexibleSpace();
                EditorGUILayout.EndHorizontal();
        ...
        private void WaterGeneratorSettings(ref WaterGenerator module)
        {
            ...
            if (THelpersUI.Foldout("LAKES/RIVERS SETTING", ref module.uIToggles.Settings1))
            {
                ...
                if (module.GenerateRiver) module.RiverWidthInMeter = THelpersUI.GUI_FloatField(new GUIContent("RIVER WIDTH (M)", "Set default width for rivers in meters"), module.RiverWidthInMeter, -10);
