...
#pragma warning restore CS4014 // Because this call is not awaited, execution of the current method continues before the call is completed
        ...
        public void RunAllModules(TTerrain terrain)
        {
            ...
            float progressSlide = TTerrainGenerator.GetProgressSlice();
            ...
            lastNodes = TTerraWorld.WorldGraph.heightmapGraph.RunGraph(terrain, ConnectionDataType.HeightmapMaster, progressSlide);
            ...
            lastNodes = TTerraWorld.WorldGraph.biomesGraph.RunGraph(terrain, ConnectionDataType.Lakes, progressSlide);
            if (!TTerrainGenerator.WorldInProgress) return;
            ...
            //   RenderingNode renderingNode = TTerraWorld.WorldGraph.renderingGraph.GetEntryNode();
            //RenderingNode renderingNode = TerrainRenderingManager.RenderingParams;

            // if (renderingNode.renderingParams.modernRendering)
            {
                lastNodes = TTerraWorld.WorldGraph.colormapGraph.RunGraph(terrain, ConnectionDataType.ColormapMaster, progressSlide);
                if (!TTerrainGenerator.WorldInProgress) return;
                ...
                lastNodes = TTerraWorld.WorldGraph.colormapGraph.RunGraph(terrain, ConnectionDataType.DetailTextureMaster, progressSlide);
                if (!TTerrainGenerator.WorldInProgress) return;
          ...
          // }
            ...
            lastNodes = TTerraWorld.WorldGraph.biomesGraph.RunGraph(terrain, ConnectionDataType.Mesh, progressSlide);
            if (!TTerrainGenerator.WorldInProgress) return;
            ...
            lastNodes = TTerraWorld.WorldGraph.biomesGraph.RunGraph(terrain, ConnectionDataType.ObjectScatter, progressSlide);
            if (!TTerrainGenerator.WorldInProgress) return;
            ...
            lastNodes = TTerraWorld.WorldGraph.biomesGraph.RunGraph(terrain, ConnectionDataType.InstanceScatter, progressSlide);
            if (!TTerrainGenerator.WorldInProgress) return;
            ...
            lastNodes = TTerraWorld.WorldGraph.biomesGraph.RunGraph(terrain, ConnectionDataType.GrassScatter, progressSlide);
            if (!TTerrainGenerator.WorldInProgress) return;
...
#if TERRAWORLD_XPRO
        public void RunAllXModulesASync(TTerrain terrain)
        {
            TDebug.TraceMessage();

            try
            {
                RunAllXModules(terrain);
            }
            catch (Exception e)
            {
                exception = e;
        ...
        public void RunAllXModules(TTerrain terrain)
        {
            TDebug.TraceMessage();
            float progressSlide = TTerrainGenerator.GetProgressSlice();
            if (terrain.map.Heightmap.heightsData == null) throw new Exception("Internal Error : 02");
            ...
            if (XcolorMap != null && renderingNode.renderingParams.modernRendering)
            {
...
#if TERRAWORLD_XPRO
            if (TTerraworldGenerator.XGraph.GetNodesCount() > 0)
                await Task.Run(() => RunAllXModulesASync(terrain));
            else
#endif
            if (TTerraWorld.WorldGraph.GetNodeCounts() > 0)
