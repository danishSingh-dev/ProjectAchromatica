...
namespace TerraUnity.Edittime
{
    ...
    public static class TTerrainGenerator
    {
        ...
        public static void CreateWorld(Action<TTerrain, Terrain, List<GameObject>, List<GameObject>> onFinished)
        {
            ...
            {
                TDebug.TraceMessage();
                nodesCount = TTerraWorld.WorldGraph.GetNodeCounts();
#if TERRAWORLD_XPRO
                xNodesCount = TTerraworldGenerator.XGraph.GetNodesCount();
        ...
        private static void InitBGTerrain()
        {
            ...
            if (TerrainRenderingManager.BGMountains)
            {
                ...
                BGArea.SetBBox(BGArea._areaSizeLat * TerrainRenderingManager.BGTerrainScaleMultiplier, BGArea._areaSizeLon * TerrainRenderingManager.BGTerrainScaleMultiplier);
                int BGTZoomLevelImagery = TMap.GetZoomLevel(TerrainRenderingManager.BGTerrainSatelliteImageResolution, BGArea);
                int BGTZoomLevelElevation = TMap.GetZoomLevel(TerrainRenderingManager.BGTerrainHeightmapResolution, BGArea);
        ...
        private static void CreateBackgroundTerrainData(TMap BGMap)
        {
            ...
            BGTData.heightmapResolution = TerrainRenderingManager.BGTerrainHeightmapResolution;
            heightmap = THeightmapProcessors.ResampleHeightmap(heightmap, THeightmapProcessors.ResampleMode.DOWN, TerrainRenderingManager.BGTerrainHeightmapResolution + 1);
            ...
            bgImage = new TImage(TImageProcessors.ResetResolution(BGMap.Image.Image, TerrainRenderingManager.BGTerrainSatelliteImageResolution));
        ...
        private static void CreateBackgroundTerrainObject()
        {
            ...
            TerrainRenderingManager.SetTerrainMaterialDefaultBG(BGTerrain);

            BGTerrain.heightmapPixelError = TerrainRenderingManager.BGTerrainPixelError;
            ...
            // We can expose parameters later in UI (Currently Snow on/off is exposed in Background Terrain section in Global Settings tab)
         //   TerrainRenderingManager.SetTerrainMaterial
         //   (
         //       BGTerrain,
         //       TTerraWorld.GeneratedTerrainsPath + "BGTerrain.mat",
         //       renderingParams.modernRendering,
         //       false,
         //       false,
         //       false,
         //       TTerraWorld.WorldGraph.FXGraph.GetEntryNode().fxParams.BGTerrainHasSnow,
         //       false,
         //       renderingParams.isFlatShading,
         //       null
         //   );
#else
         //   TerrainRenderingManager.SetTerrainMaterial(BGTerrain, TTerraWorld.GeneratedTerrainsPath + "BGTerrain.mat");
#endif

            BGTerrainGO.transform.parent = TTerraWorldManager.MainTerrainGO.transform;
            BGTerrainGO.transform.position = new Vector3(-(BGTerrain.terrainData.size.x / 2f), TerrainRenderingManager.BGTerrainOffset, -(BGTerrain.terrainData.size.z / 2f));
        }
        ...
        private static Terrain InitializeTerrain(TTerrain TWTerrainData , int index)
        {
            ...
            _mainTerrainGO.transform.parent = TTerraWorldManager.MainTerraworldGameObject.transform;
            ...
            TerrainRenderingManager.SetMainTerrainMaterialDefault(terrain);
...
#if TERRAWORLD_PRO
            //Texture2D overallWaterMask = null;

            if (TWTerrainData.overallWaterMask != null)
            {
                //overallWaterMask = TWTerrainData.overallWaterMask.GetTexture("WaterMask");
                TerrainRenderingManager.WaterMaskTexture = TWTerrainData.overallWaterMask.GetTexture("WaterMask");
            }

            //   TerrainRenderingManager.SetTerrainMaterial
            //   (
            //       terrain,
            //       TTerraWorld.GeneratedTerrainsPath + "Terrain.mat",
            //       renderingParams.modernRendering,
            //       renderingParams.tessellation,
            //       renderingParams.heightmapBlending,
            //       renderingParams.colormapBlending,
            //       renderingParams.proceduralSnow,
            //       renderingParams.proceduralPuddles,
            //       renderingParams.isFlatShading,
            //       overallWaterMask
            //   );
#else
         //   TerrainRenderingManager.SetTerrainMaterial(terrain, TTerraWorld.GeneratedTerrainsPath + "Terrain.mat");
#endif
        ...
        public static void UpdateTerrainImagery(TTerrain tTerrainData, Terrain terrain)
        {
            ...
            if (!TerrainRenderingManager.SplatmapResolutionBestFit)
                tData.alphamapResolution = TerrainRenderingManager.SplatmapResolution;
            else
        ...
        public static void ApplyLakeOnTerrain(Terrain terrain, TTerrain tTerrainData, out List<GameObject> lakeList)
        {
            ...
            if (_lakeLayers?.Count != 0)
            {
                ...
                //terrainManagerScript.OverAllWaterMask = tTerrainData.overallWaterMask.GetTexture("WaterMask");
...
//                        pivot.y += offsetForNegetiveHeightHandling * TTerraWorld.WorldGraph.heightmapGraph.HeightmapSource().elevationExaggeration;
        ...
        {
            float progressSlide = 100 / ((nodesCount * _terraWorld._terrains.Count) + 1);
