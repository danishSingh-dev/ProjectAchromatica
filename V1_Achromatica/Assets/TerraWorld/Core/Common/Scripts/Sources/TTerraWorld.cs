...
namespace TerraUnity.Edittime
{
   ...
   //     public int BgTerrainPixelError { get => WorldGraph.renderingGraph.GetEntryNode().renderingParams.BGTerrainPixelError; }
        ...
        {
            TAreaGraph OldareaGraph = null;

            if (_worldGraph != null)
                OldareaGraph = _worldGraph.areaGraph;

            _worldGraph = TTerraWorldGraph.GetNewWorldGraph(TVersionController.MajorVersion, TVersionController.MinorVersion);

            if (OldareaGraph != null)
                _worldGraph.areaGraph = OldareaGraph;

            SaveWorldGraph();
        }

        public static void SaveWorldGraph(string path = "")
        {
            ...
            {
                path = WorkDirectoryFullPath + "graph.xml";
                _worldGraph.SaveGraph(path);
        ...
        public static void LoadWorldGraph(string path, bool Template, out Exception exception, out bool reGenerate)
        {
            ...
            try
            {
                ...
                {
                    if (_worldGraph == null) _worldGraph = new TTerraWorldGraph();
                    reGenerate = _worldGraph.LoadGraph(path, Template);
                    SaveWorldGraph();
                }
                else
