...
namespace TerraUnity.Edittime
{
    public class TTerraworldGenerator //: MonoBehaviour
    {
        ...
        private static void SaveXGraph(TXGraph XGraph)
        {
            string savedPath = TAddresses.WorkDirectoryPath + "xgraph.asset";
            if (File.Exists(savedPath)) AssetDatabase.DeleteAsset(savedPath);
            AssetDatabase.CreateAsset(XGraph, savedPath);
        ...
        private static TXGraph GetXGraph()
        {
            ...
            {
                string savedPath = TAddresses.WorkDirectoryPath + "xgraph.asset";
                if (!File.Exists(savedPath)) ResetXGraph();
                TXGraph _xGraph = (TXGraph)AssetDatabase.LoadAssetAtPath(savedPath, typeof(TXGraph));
                return _xGraph;
            }
            catch (Exception)
            {
        ...
        {
            TXGraph _xGraph = TTerraWorldGraph.GetNewXGraph(TTerraWorld.MajorVersion, TTerraWorld.MinorVersion);
            SaveXGraph(_xGraph);
        }
...
#pragma warning restore CS4014 // Because this call is not awaited, execution of the current method continues before the call is completed
        ...
        public static void UpdateInstancesLayer(Terrain terrain, TTerrain tTerrainData)
        {
            try
            {
                ...
                {
                    if (t == null) continue;
                    TScatterParams terraLayerManager = t?.GetComponent<TScatterParams>();

                    //TODO: Just for Ver 1.15 - It should be overridden in next version  
                    if (terraLayerManager != null)
                        terraLayerManager.UpdateLayer();
                }
            }
            catch (Exception e)
            {
        ...
        public static void FinalizeWorld(Terrain terrain)
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(TTerraWorld.TemplateName))
                {
                    ...
                    //RenderingNode rnNode = TTerraWorld.WorldGraph.renderingGraph.GetEntryNode();
