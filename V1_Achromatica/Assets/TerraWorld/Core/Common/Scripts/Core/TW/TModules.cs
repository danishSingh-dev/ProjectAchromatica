...
namespace TerraUnity.Edittime
{
    ...
    public class TGraph
    {
        ...
        public List<TNode> RunGraph(TTerrain CurrentTerrain, ConnectionDataType outputDataType, float progressSlide)
        {
            TDebug.TraceMessage();
            List<TNode> result = new List<TNode>();
            List<TSequence> actionSequences = new List<TSequence>();
                ...
                if (sequence.Count > 0)
                {
                    for (int i = 0; i < sequence.Count; i++)
                    {
                        if (TTerrainGenerator.WorldInProgress) 
                        {
                         ...
                         //   CurrentTerrain.Progress += progressSlide;
                        }
                    }
                }
    ...
    public abstract class TNode
    {
        ...
        public string GetMainResource()
        {
            ...
            try
            {
                List<string> vs = GetResourcePaths();

                if (vs == null || vs.Count == 0) 
                    result = "-";
                else
                    result = Path.GetFileNameWithoutExtension(vs[0]);
            }
            catch (Exception)
            {
                result = ".";
