        ...
        public override void OnInspectorGUI ()
        {
            ...
            for (int i = 0; i < InteractiveTargets.playerTargets.Count; i++)
            {
                ...
                if (playerInteractions.GPULayers != null && playerInteractions.GPULayers.Count > 0)
                {
                    ...
                    {
                        playerInteractions.maximumObjects = THelpersUIRuntime.GUI_IntSlider(new GUIContent("MAXIMUM INTERACTIVE OBJECTS", "Maximum count for interactive objects around target for each layer which will be updated dynamically in runtime"), playerInteractions.maximumObjects, 1, 128, 20);
                    ...
                    for (int j = 0; j < playerInteractions.GPULayers.Count; j++)
                    {
                        ...
                        state = THelpersUIRuntime.GUI_SelectionGrid(state, convertSelection, new GUIStyle(EditorStyles.toolbarButton), -10);
