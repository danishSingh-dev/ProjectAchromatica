    ...
    public class TBrushFunctions : Editor
    {
        ...
        public static void MaskEditorSceneGUI ()
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        terrainWidth = GL.MGP.terrain.terrainData.size.x;
                        terrainlength = GL.MGP.terrain.terrainData.size.z;
                        terrainPosition = new Vector2(GL.MGP.terrain.transform.position.x, GL.MGP.terrain.transform.position.z);
                ...
                if (Tools.current != Tool.View)
                {
                    ...
                    else if (WorldToolsParams.e.rawType == EventType.MouseUp && WorldToolsParams.maskIsDirty)
                    {
                        if (WorldToolsParams.globalMode)
                        {
                            ...
                            if (WT.affectGPULayers && WT.maskDataListGPU != null)
                            {
                                ...
                                if (GPUProfiles != null && GPUProfiles.Count > 0 && GPUProfiles[0] != null)
                            ...
                            if (WT.affectGrassLayers && WT.maskDataListGrass != null)
                            {
                            ...
                            DetectTerrainChanges.SyncLayersWithTerrain(WorldToolsParams.e.mousePosition, WT.affectGPULayers, WT.affectGrassLayers, lastFilters);
                        }
                        else
                        {
                            ...
                            if (WorldToolsParams.isGPULayer)
                            {
                                DetectTerrainChanges.SyncLayerWithTerrain(WorldToolsParams.e.mousePosition, GIL.parameters.maskData);
                                lastMousePosition = WorldToolsParams.e.mousePosition;
                                lastFilter = GIL.parameters.maskData;
                            }
                            else if (WorldToolsParams.isGrassLayer)
                            {
                                DetectTerrainChanges.SyncLayerWithTerrain(WorldToolsParams.e.mousePosition, GL.MGP.maskData);
                                lastMousePosition = WorldToolsParams.e.mousePosition;
        ...
        private static void DetectPaintingChanges ()
        {
            if (Undo.GetCurrentGroupName() == worldPaintingLayers)
                DetectTerrainChanges.SyncLayersWithTerrain(true, true, lastFilters);

            if (Undo.GetCurrentGroupName() == worldPaintingLayerGPU || Undo.GetCurrentGroupName() == worldPaintingLayerGrass)
                DetectTerrainChanges.SyncLayerWithTerrain(lastMousePosition, lastFilter);
        }
            ...
            try
            {
                ...
                if (isGrass)
                {
                    ...
                    foreach (GrassLayer t in grassLayers)
                        ...
                        {
                            if (exception != null && t.transform == exception) continue;
                            t.active = WorldToolsParams.cachedStatesGrass[index];
        ...
        private static void OnEditorUpdate()
        {
            if (WorldToolsParams.applySyncing && Time.realtimeSinceStartup - WorldToolsParams.lastEditorUpdateTime > WorldToolsParams.syncDelay)
            {
                ...
                DetectTerrainChanges.SyncLayersWithTerrain(WorldToolsParams.syncAllGPU, WorldToolsParams.syncAllGrass, filters);
