...
namespace TerraUnity.Runtime
{
    ...
    public class TimeOfDay : MonoBehaviour
    {
        ...
        public bool enableTimeOfDay = true;
        public Gradient nightDayColor;
        public float sunPosNormalized;
        ...
        public void UpdateAtmosphere(bool forced = false)
        {
            ...
            if (!forced)
            {
                runOnceToggle = !IsDynamicSky();
                if (runOnceToggle) return;
            }
            ...
            {
                if (enableTimeOfDay)
                    sun.transform.Rotate(dayRotateSpeed * Time.deltaTime * skySpeed);
                else
            ...
            {
                if (enableTimeOfDay)
                    sun.transform.Rotate(nightRotateSpeed * Time.deltaTime * skySpeed);
                else
                    sun.transform.rotation = Quaternion.Euler(new Vector3(globalTimeManager.Elevation, globalTimeManager.Azimuth, 0));
            ...
            // Update scene's Ambient Light when needed
            if (forced || !runOnceToggle)
                DynamicGI.UpdateEnvironment();
            else if (previousRotation != sun.transform.rotation)
            {
                if (gradualEnvironmentLightingUpdate && Time.realtimeSinceStartup % 2 == 0)
                {
                    DynamicGI.UpdateEnvironment();
                    previousRotation = sun.transform.rotation;
                }
                else
                {
                    DynamicGI.UpdateEnvironment();
