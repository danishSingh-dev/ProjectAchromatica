...
float waterMask = 0;
	...
	Puddles(puddle, o.Albedo, o.Normal, o.Smoothness, o.Metallic, _Smoothness, _Metallic, finalCol.rgb, waterMask);

	#ifdef _PROCEDURALSNOW
		if (puddle > _Slope || puddle < _SlopeMin)
		{
			Snow(o.Albedo, o.Smoothness, o.Normal, _worldNormal, IN.uv_Control, IN.color.y, IN.worldPos.y, finalCol.rgb, _Smoothness, waterMask);
			o.Metallic = _Metallic * 4;
		}
	#endif
#else
	#ifdef _PROCEDURALSNOW
		Snow(o.Albedo, o.Smoothness, o.Normal, _worldNormal, IN.uv_Control, IN.color.y, IN.worldPos.y, finalCol, _Smoothness, waterMask);
	#else
		o.Albedo = finalCol;
